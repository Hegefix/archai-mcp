...
export function registerCreateFolder(server: McpServer, registry: VaultRegistry): void {
  server.registerTool(
    "create_folder",
    {
      description:
        "Create a folder (and any parent folders) in an Obsidian vault. Idempotent: returns created=false when the folder already exists. Errors if the target path collides with a regular file. Rejects absolute paths and paths that escape vault root.",
      inputSchema: {
        path: z
          .string()
          .describe(
